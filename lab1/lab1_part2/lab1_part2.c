...
static void vRgbTask(void *pvParameters)
{
    ...
    while (1){ 
        ...
        // if (state == PUSH_DECREMENT_STATE && previous_state != state && xDelay > 0) {
        //     xDelay -= increment;
        //     xil_printf("\nxPeriod: %d\n", xDelay * 2);
        //     vTaskDelay(inputDelay);
        // } else if (state == PUSH_INCREMENT_STATE && previous_state != state) {
        //     xDelay += increment;
        //     xil_printf("\nxPeriod: %d\n", xDelay * 2);
        //     vTaskDelay(inputDelay);
        // }
        if (state == PUSH_DECREMENT_STATE && xOnDelay > 0 && previous_state != state && xDelay > 0) {
            xOnDelay -= increment;
            xOffDelay = xPeriod - xOnDelay;
            xil_printf("\nxOnDelay: %d\txOffDelay: %d\n", xOnDelay, xOffDelay);
            vTaskDelay(inputDelay);
        } else if (state == PUSH_INCREMENT_STATE && xOffDelay > 0 && previous_state != state) {
            xOnDelay += increment;
            xOffDelay = xPeriod - xOnDelay;
            xil_printf("\nxOnDelay: %d\txOffDelay: %d\n", xOnDelay, xOffDelay);
            vTaskDelay(inputDelay);
