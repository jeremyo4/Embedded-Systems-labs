...
int main(void)
{
    ...
    xTaskCreate(vUartManagerTask, "UART", 1024, NULL, 2, NULL);
    xTaskCreate(vSpiMainTask, "SPI_MAIN", 1024, NULL, 2, NULL);
    xTaskCreate(vSpiSubTask, "SPI_SUB", 2048, NULL, 2, NULL);
    xTaskCreate(vRgbLedTask, "RGB", 512, NULL, 2, NULL);
    ...
    while (1) {
        //TODO 14: send $ until a $ is received
         if (report_flag) {
            ...
            if (xQueueReceive(spi_to_uart, &spi_byte, 0) == pdTRUE) {
        ...
        if (spi_loopback && command_flag == 2) {
            ...
            for (i = 0; i < TRANSFER_SIZE_IN_BYTES; i++) {
				...
				if (terminationSequence(rolling)) {
                    ...
                    report_len = snprintf(report, sizeof(report), "\nNumber of bytes received over SPI:%d\nLast message byte count: %d\nTotal messages received: %d\n", total_bytes_received_over_spi - (3*total_messages_received), last_message_byte_count - 3, total_messages_received);
