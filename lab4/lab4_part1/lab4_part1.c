...
int emergency_flag = 0; // emergency flagged when non-zero
...
static void _Task_Emerg_Stop( void *pvParameters ){
	...
	while(1){
		...
		if(pressedCount >= 3){
            ...
            emergency_flag = 1; // programs responding to this flag shall now cease
            sequenceIndex = 0;
            Stepper_SetupStop();
            Stepper_disableMotor();
            ...
            for (;;) {
                ...
                vTaskDelay(pdMS_TO_TICKS(333.3/2));    // 3 Hz x 2
                XGpio_DiscreteWrite(&Red_RGBInst, 2, 0b000);
                vTaskDelay(pdMS_TO_TICKS(333.3/2));    // 3 Hz x 2
                Stepper_disableMotor();
            }
